...
import { blankCubeFSText, blankCubeVSText, meshVSText, meshFSText } from "./Shaders.js";
import { Vec4, Vec3 } from "../lib/TSM.js";
import { Quat } from "../lib/tsm/Quat.js";
import { RenderPass } from "../lib/webglutils/RenderPass.js";
import { Cube, typeToColor } from "./Cube.js";
import { Chunk } from "./Chunk.js";
import { CLoader } from "./AnimationFileLoader.js";
import { makeEmptyInventory, addItem, ITEM_DATA, isSlotEmpty, itemTypeEnum } from "./Inventory.js";
const chunkSize = 64;
const chunksToLoad = 1;
export class MinecraftAnimation extends CanvasAnimation {
    constructor(canvas) {
        super(canvas);
        this.playerHealth = 20; // 20 halves = 10 full hearts
        this.playerMaxHealth = 20;
        /* Mesh Rendering */
        this.meshRenderPasses = new Map();
        this.wolfMesh = null;
        this.wolfLoader = null;
        this.SLOT_SIZE = 70;
        this.GAP = 6;
        this.HOTBAR_Y_OFFSET = 300;
        this.INVENTORY_VERTICAL_OFFSET = 30;
        this.craftingOutputSlot = { itemType: null, count: 0 };
        this.dragState = null;
        this.lastClick = { time: 0, slot: null };
        this.canvas2d = document.getElementById("textCanvas");
        this.hudContext = this.canvas2d.getContext("2d");
    ...
    rayIntersectsAABB(rayOrigin, rayDir, boxMin, boxMax) {
        let tmin = -Infinity;
        let tmax = Infinity;
        const axes = ['x', 'y', 'z'];
    ...
    onInventoryMouseDown(button, shift, mx, my) {
        ...
        if (shift && (now - this.lastClick.time < 300) && this.lastClick.slot === hovered.slot) {
            if (!isSlotEmpty(held))
                this.consolidateItems(held.itemType);
            return;
        }
    ...
    handleSlotClick(button, shift, hovered) {
        const slot = hovered.slot;
        const held = this.heldItem;
    ...
    takeCraftingOutput(shift) {
        if (isSlotEmpty(this.craftingOutputSlot))
            return;
    ...
    evaluateCrafting() {
        ...
        let dirtCount = 0;
        for (const slot of this.craftingSlots) {
            if (slot.itemType === itemTypeEnum.DIRT_BLOCK)
                dirtCount++;
        }
        if (dirtCount === 4) {
    ...
    drawScene(x, y, width, height) {
        ...
        let offsetCol = 0;
        let offsetType = 0;
        let allMobs = [...this.mobs]; // FIX: this is just for the one pink test mob for now - need to switch to mesh files later
        for (const chunk of this.chunks.values()) {
    ...
    drawSlotContents(context, slot, x, y, slotSize) {
        if (isSlotEmpty(slot) || slot.itemType === null)
            return;
    ...
    drawInventorySlot(x, y, slotIndex, selected, context, slotSize) {
    ...
    drawPanel(context, x, y, w, h, fill = "rgba(45,45,45,0.94)", stroke = "rgba(180,180,180,0.9)") {
        context.save();
    ...
    drawSlotFrame(context, x, y, size, selected = false) {
    ...
    getInventoryScreenLayout(canvas) {
        const slotSize = this.SLOT_SIZE;
        const gap = this.GAP;
    ...
    drawPlayerAvatar(context, x, y, w, h) {
        ...
        context.fillRect(cx - 18, baseY - 148, 36, 10);
        context.fillStyle = "#d7ab84";
        context.fillRect(cx - 18, baseY - 138, 36, 34);
        ...
        // Rest of body
        context.fillStyle = "#018883";
        context.fillRect(cx - 18, baseY - 104, 36, 44);
        context.fillStyle = "#018883";
        context.fillRect(cx - 34, baseY - 102, 14, 26);
        context.fillRect(cx + 20, baseY - 102, 14, 26);
        context.fillStyle = "#d7ab84";
        context.fillRect(cx - 34, baseY - 76, 14, 34);
        context.fillRect(cx + 20, baseY - 76, 14, 34);
        context.fillStyle = "#3b2e88";
        context.fillRect(cx - 18, baseY - 60, 15, 42);
        context.fillRect(cx + 3, baseY - 60, 15, 42);
        context.fillStyle = "#555";
        context.fillRect(cx - 18, baseY - 18, 15, 8);
        context.fillRect(cx + 3, baseY - 18, 15, 8);
    }
    drawCraftingSection(context, layout, title, slots) {
    ...
    drawInventoryGrid(context, layout) {
        const selected = this.gui.getSelectedHotbarSlot();
        const slotSize = this.SLOT_SIZE;
        const gap = this.GAP;
        context.fillStyle = "white";
    ...
    drawFullInventory(context, canvas) {
    ...
    drawHeldItem(context) {
        if (isSlotEmpty(this.heldItem) || this.heldItem.itemType === null)
            return;
        const mouseX = this.gui.getMouseX();
        const mouseY = this.gui.getMouseY();
        const slotSize = 70;
    ...
    buildMeshRenderPass(mesh) {
        ...
        pass.addAttribute("aVertPos", 3, gl.FLOAT, false, 3 * Float32Array.BYTES_PER_ELEMENT, 0, undefined, geo.position.values);
        pass.addAttribute("aNorm", 3, gl.FLOAT, false, 3 * Float32Array.BYTES_PER_ELEMENT, 0, undefined, geo.normal.values);
        pass.addInstancedAttribute("aInstOffset", 3, gl.FLOAT, false, 3 * Float32Array.BYTES_PER_ELEMENT, 0, undefined, new Float32Array(0));
        pass.addInstancedAttribute("aInstRot", 4, gl.FLOAT, false, 4 * Float32Array.BYTES_PER_ELEMENT, 0, undefined, new Float32Array(0));
        pass.addUniform("uLightPos", (gl, loc) => {
            gl.uniform4fv(loc, this.lightPosition.xyzw);
        });
        pass.addUniform("uProj", (gl, loc) => {
            gl.uniformMatrix4fv(loc, false, new Float32Array(this.gui.projMatrix().all()));
        });
        pass.addUniform("uView", (gl, loc) => {
            gl.uniformMatrix4fv(loc, false, new Float32Array(this.gui.viewMatrix().all()));
        });
    ...
    drawMesh(mesh, positions, rotations) {
...
export function coordToChunk(coord) {
