...
import { Mesh, BoneTrack, AnimationClip } from "./Scene.js";
...
export class BoneLoader {
    constructor(name, parentId, childrenIds, offset, wmat) {
        this.name = name;
...
export class MeshLoader {
    constructor(skinnedMesh) {
        ...
        skinnedMesh.skeleton.bones.forEach(bone => {
            ...
            this.bones.push(new BoneLoader(bone.name, parentId, children, yVal, tempMat));
        });
...
class CLoader {
    ...
    load(callback) {
        this.loader.load(this.fileLocation, (collada) => {
            ...
            // Attach animation clips from the Collada file to each Mesh.
            // ColladaLoader exposes animations as THREE.AnimationClip[] with tracks
            // named like "BoneName.quaternion" / ".position" / ".scale". Only
            // quaternion (rotation) tracks are honored here.
            const tAnims = (_a = collada.animations) !== null && _a !== void 0 ? _a : [];
            if (tAnims.length > 0) {
                this.meshes.forEach(mesh => {
                    const nameToIdx = new Map();
                    mesh.bones.forEach((b, i) => nameToIdx.set(b.name, i));
                    tAnims.forEach(tAnim => {
                        var _a, _b, _c;
                        const tracks = new Array(mesh.bones.length).fill(null);
                        let hadAny = false;
                        ((_a = tAnim.tracks) !== null && _a !== void 0 ? _a : []).forEach((tTrack) => {
                            var _a;
                            const tname = (_a = tTrack.name) !== null && _a !== void 0 ? _a : "";
                            const stripped = tname.startsWith(".") ? tname.slice(1) : tname;
                            const match = stripped.match(/^(.*)\.quaternion$/);
                            if (!match)
                                return;
                            const boneName = match[1];
                            const idx = nameToIdx.get(boneName);
                            if (idx === undefined)
                                return;
                            tracks[idx] = new BoneTrack(new Float32Array(tTrack.times), new Float32Array(tTrack.values));
                            hadAny = true;
                        });
                        if (hadAny) {
                            mesh.animations.push(new AnimationClip((_b = tAnim.name) !== null && _b !== void 0 ? _b : "", (_c = tAnim.duration) !== null && _c !== void 0 ? _c : 0, tracks));
                        }
                    });
                });
            }
