...
export function randomOctaveNoise(xStart: number, yStart: number): number[][] {
...
function getNoiseForSingleOctave(octaveSize: number, xStart: number, yStart: number): number[][] {
    ...
    for (let x = 0; x < startSize; x+=1) {
        for (let y = 0; y < startSize; y+=1) {
            ...
            noise[x][y] = hashToReal(randomize("octave" + octaveSize.toString(),
                hash3D(xStart + (x * octaveSize) - octaveSize, yStart + (y * octaveSize) - octaveSize, octaveSize)));
        }
    }
...
function upSampleNoise(noise: number[][]): number[][] {
