...
export class BoneLoader {
  ...
  constructor(name: string, parentId: number, childrenIds: number[], offset: number, wmat: Mat4) {
    this.name = name;
...
class CLoader {
  ...
  public load(callback: Function): void {
    this.loader.load(this.fileLocation, (collada: Collada) => {
      ...
      const tAnims: any[] = (collada as any).animations ?? [];
      if (tAnims.length > 0) {
        this.meshes.forEach(mesh => {
          const nameToIdx = new Map<string, number>();
          mesh.bones.forEach((b, i) => nameToIdx.set(b.name, i));
          tAnims.forEach(tAnim => {
            const tracks: (BoneTrack | null)[] = new Array(mesh.bones.length).fill(null);
            let hadAny = false;
            (tAnim.tracks ?? []).forEach((tTrack: any) => {
              const tname: string = tTrack.name ?? "";
              ...
              tracks[idx] = new BoneTrack(
                new Float32Array(tTrack.times),
                new Float32Array(tTrack.values),
              );
              hadAny = true;
            });
            if (hadAny) {
              mesh.animations.push(new AnimationClip(tAnim.name ?? "", tAnim.duration ?? 0, tracks));
            }
          });
        });
      }
